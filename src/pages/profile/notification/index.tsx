...
import { Button, Checkbox, Stack, Switch } from '@mui/material';
...
const Notification = () => {
  ...
  const CheckBoxGroup = ({ title, name }: { title: string; name: string }) => (
    ...
    >
      ...
      />
      <Typography variant="caption" color={'#4A5567'} fontWeight={500} fontSize={'15px'}>
        {title}
      </Typography>
    </Box>
  );
      ...
      <MainCard>
        <Stack marginBottom={4}>
          <Stack direction={'row'} justifyContent={'space-between'} alignItems={'center'}>
            <Typography sx={{ fontSize: '19px', color: '#394663', fontWeight: '600' }}>Notification Settings</Typography>
          ...
          <Typography variant="caption" color={'#778194'} fontWeight={400} fontSize={'14px'} width={670}>
            Set your notification preferences for various features and activities. This setting will enable both the email notification and
          ...
          <Stack direction={'row'} justifyContent={'space-between'}>
            <Stack width={450}>
              <Stack direction={'row'} justifyItems={'center'} gap={2}>
                ...
                <Stack>
                  <Typography fontSize={'14px'} color={'#394663'} fontWeight={600}>
                    Email Notifications
                  </Typography>
                  <Typography variant="caption">Receive notifications via email</Typography>
                </Stack>
              </Stack>
            </Stack>
            <Stack direction={'row'} alignItems={'center'}>
        ...
        <Box sx={{ border: '1px solid #C8D7F7', borderRadius: 1, padding: 2, marginTop: 6 }}>
          <Stack marginBottom={3}>
            <Typography fontSize={'20px'} sx={{ color: '#2A50ED' }} fontWeight={600}>
              Student Platform
            </Typography>
            <Typography variant="caption" color={'#4A5567'} fontSize={'15px'} fontWeight={500}>
              Select the kind of notifications you get about various activities. Below notifications will be sent to your registered email
              address or student’s registered email address
            </Typography>
          </Stack>
          ...
          />
          <CheckBoxGroup name="notification2" title="Notification to student when you submit one or more applications for the student." />
          <CheckBoxGroup
            name="notification3"
          ...
          <CheckBoxGroup
            ...
            title="Weekly Notification to student if the student's profile is not completed after 7 days of registering the student."
          />
          <CheckBoxGroup name="notification7" title="Send me my staff’s email communications for application notes." />
        </Box>

        <Box sx={{ display: 'flex', justifyContent: 'end', marginTop: 2 }}>
          <LoadingButton loading={loading} size="medium" type="submit" variant="contained" color="primary" onClick={submit}>
            Save
          </LoadingButton>
        </Box>
      </MainCard>
