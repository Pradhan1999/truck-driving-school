...
export default function Breadcrumbs({
  ...
  if ((item && item.type === 'item') || (item?.type === 'group' && item?.url) || custom) {
    ...
    itemContent = (
      <Typography variant="body1" color="text.secondary" sx={{ fontSize: 12, display: 'flex', alignItems: 'center' }}>
    ...
    let tempContent = (
      <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon}>
        <Typography
          ...
          variant="h6"
          sx={{ textDecoration: 'none', display: 'flex', alignItems: 'center', fontSize: 12 }}
        >
          {icons && <Home3 style={iconSX} />}
          {icon && !icons && <Home3 variant="Bold" style={{ ...iconSX, marginRight: 0 }} />}
          {(!icon || icons) && 'Home'}
